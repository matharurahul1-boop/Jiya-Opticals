...
export function TeamManager() {
  ...
  if (currentUser.role !== 'Admin') return <div className="p-6">{t("Only admins can manage team access.")}</div>;
  if (!ownerId) return <div className="p-6">{t("Sign in with Supabase to manage team access.")}</div>;
  ...
  return (
    <section className="p-4 md:p-6 max-w-5xl mx-auto space-y-5">
      ...
      >
        <h2 className="font-bold">{t("Assign a team member")}</h2>
        <p className="text-sm text-stone-600">
          {t("Enter the email they will sign up with, then pick their shops. No invitation email is sent from here — share the app link and ask them to sign up with this exact email. ")}</p>
        <fieldset disabled={busy} className="space-y-4">
          <label className="block">
            {t("Member email ")}<input
              required
              type="email"
              value={email}
          ...
          <legend className="font-medium">{t("Assigned shops")}</legend>
        ...
        {!shops.length && (
          <button type="button" className="underline" onClick={() => setActiveTab('shops')}>
            {t("Create a shop first ")}</button>
        )}
        {saveStatus !== 'Saved to Supabase' && (
          <p className="text-amber-800 text-sm">{t("Wait for store changes to save before assigning access.")}</p>
        )}
        <button
        ...
        >
          {busy ? t("Saving…") : t("Save assignment")}
        </button>
      </form>

      <div className="bg-white border rounded-2xl overflow-hidden">
        <h2 className="font-bold p-4 border-b">{t("Team members")}</h2>
        {loading ? (
          <p className="p-4">{t("Loading…")}</p>
        ) : !members.length ? (
          <p className="p-4 text-stone-500">{t("No team members assigned yet.")}</p>
        ) : (
          members.map((member) => (
              ...
              <div className="min-w-0">
                ...
                <span className="text-xs text-stone-500">
                  {member.isAdmin
                    ...
                    : member.registered
                    ? t("Email verified")
                    : t("Awaiting sign-up / email verification")}
                </span>
              </div>
