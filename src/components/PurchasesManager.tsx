    ...
    <div id="purchases-manager-container" className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-amber-200/80 p-4 rounded-xl shadow-xs">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-stone-900 flex items-center gap-2">
              {t("Supplier Purchases & Inward Stock ")}<span className="text-[10px] bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded font-medium">
                {t("PURCHASE REGISTER ")}</span>
            </h1>
            <p className="text-xs text-stone-500">
              {t("Record vendor bills and manage supplier ledgers. ")}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <Plus className="w-4 h-4" /> {t("Record Purchase Bill ")}</button>
        </div>
      </div>
      ...
      <div className="bg-white border border-amber-200/80 rounded-xl p-4 space-y-3 shadow-xs">
        <span className="text-xs font-bold text-stone-700 uppercase tracking-wider block">
          {t("Registered Suppliers (")}{suppliers.length})
        </span>
          ...
          {suppliers.map((s) => (
            <div key={s.id} className="p-3 bg-stone-50 border border-stone-200 rounded-lg space-y-1 text-xs">
              <div className="font-bold text-stone-900 flex items-center justify-between">
                <span>{s.name}</span>
                <span className="text-[10px] text-amber-800 font-mono">{t("GST: ")}{s.gstin || 'N/A'}</span>
              </div>
              <div className="text-[11px] text-stone-600">
                {t("Contact: ")}{s.contactPerson} ({s.mobile})
              </div>
              ...
              <div className="pt-1 flex justify-between border-t border-stone-200 text-[11px]">
                <span className="text-stone-500">{t("Outstanding:")}</span>
      ...
      <div className="bg-white border border-amber-200/80 rounded-xl overflow-hidden shadow-xs space-y-3 p-4">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-stone-700 uppercase">
            {t("Inward Purchase Bills (")}{purchases.length})
          </span>
          ...
          <table className="w-full text-left text-xs text-stone-700">
            <thead className="bg-stone-50 text-stone-600 uppercase text-[10px] font-semibold border-b border-stone-200">
              <tr>
                ...
                <th className="py-2.5 px-2">{t("Supplier Name")}</th>
                <th className="py-2.5 px-2">{t("Vendor Bill #")}</th>
                <th className="py-2.5 px-2 text-center">{t("Items Qty")}</th>
                <th className="py-2.5 px-2 text-right">{t("Grand Total (₹)")}</th>
                <th className="py-2.5 px-2 text-right">{t("Paid (₹)")}</th>
                <th className="py-2.5 px-3 text-right">{t("Balance Due (₹)")}</th>
              </tr>
            </thead>
          ...
          <div className="bg-white border border-stone-200 rounded-2xl max-w-2xl w-full p-5 shadow-xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-stone-200 pb-3">
              <h3 className="text-sm font-bold text-stone-900 flex items-center gap-2">
                ...
                {t("Record Supplier Inward Purchase Bill ")}</h3>
            ...
            <form onSubmit={handleSavePurchase} className="space-y-4 text-xs">
              <label className="block p-4 bg-amber-50 rounded-xl text-amber-900 font-semibold">{t("Receiving shop ")}<select required value={receivingShopId} onChange={e=>{setReceivingShopId(e.target.value);setPurchaseItems([]);setPickedProductId('');}} className="mt-2 w-full rounded-lg border border-amber-200 bg-white p-2.5"><option value="">{t("Choose a shop")}</option>{shops.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>
                <span className="block mt-2 font-normal">{t("Stock increases only in this shop. Changing shops clears the selected items.")}</span>
              </label>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-stone-600 mb-1">{t("Select Supplier *")}</label>
                  <select
                    required
                ...
                <div>
                  <label className="block text-stone-600 mb-1">{t("Supplier Bill / Invoice #")}</label>
                  <input
                    type="text"
                    required
                    placeholder={t("e.g. LUX/2026/892")}
                ...
                <div>
                  <label className="block text-stone-600 mb-1">{t("Bill Date")}</label>
                  <input
                    type="date"
                    value={billDate}
              ...
              <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 space-y-3">
                <span className="text-xs font-bold text-amber-800 block">{t("Add Product Line Item")}</span>
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
                  <div className="sm:col-span-2">
                    <label className="block text-stone-500 text-[10px]">{t("Product")}</label>
                    <select
                  ...
                  <div>
                    <label className="block text-stone-500 text-[10px]">{t("Qty Inward")}</label>
                    <NumberInput
                      type="number"
                      min="1"
                      value={itemQty}
                  ...
                  <div>
                    <label className="block text-stone-500 text-[10px]">{t("Cost Rate (₹)")}</label>
                    <NumberInput
                      type="number"
                      min="0"
                      value={itemRate}
                ...
                {purchaseItems.length > 0 && (
                  <table className="w-full text-left text-xs border border-stone-200">
                    <thead className="bg-stone-100 text-stone-600 text-[10px]">
                      <tr>
                        <th className="p-1.5">{t("Item Name")}</th>
                        <th className="p-1.5 text-center">{t("Qty")}</th>
                        <th className="p-1.5 text-right">{t("Cost")}</th>
                        <th className="p-1.5 text-right">{t("Total (Inc. GST)")}</th>
              ...
              <div className="grid grid-cols-3 gap-3 bg-stone-50 p-3 rounded-lg border border-stone-200">
                <div>
                  <span className="text-stone-500 block text-[10px]">{t("Total Bill Amount:")}</span>
                  <span className="text-lg font-bold text-stone-900">₹{purchaseGrandTotal.toFixed(2)}</span>
                </div>

                <div>
                  <label className="block text-stone-500 text-[10px] mb-0.5">{t("Amount Paid Now (₹)")}</label>
                  <NumberInput
                    type="number"
                    min="0"
                ...
                <div>
                  <span className="text-stone-500 block text-[10px]">{t("Balance Due:")}</span>
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t border-stone-200">
                ...
                >
                  {t("Save Inward Purchase & Update Stock ")}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white border border-stone-200 rounded-xl p-5 max-w-md w-full shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-stone-200 pb-3">
              <h3 className="text-sm font-bold text-stone-900 flex items-center gap-2">
                ...
                {t("Add Optical Supplier / Distributor ")}</h3>
              <button onClick={() => setShowAddSupplierModal(false)} className="text-stone-400 hover:text-stone-600">✕</button>
            </div>

            <form onSubmit={handleCreateSupplier} className="space-y-3 text-xs">
              <div>
                <label className="block text-stone-600 mb-1">{t("Company / Supplier Name *")}</label>
                <input
                  type="text"
                  required
                  placeholder={t("e.g. Carl Zeiss Optical India")}
                  value={newSuppName}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-stone-600 mb-1">{t("Contact Person")}</label>
                  <input
                    type="text"
                    placeholder={t("Sales Rep Name")}
                ...
                <div>
                  <label className="block text-stone-600 mb-1">{t("Mobile / Phone")}</label>
                  <input
                    type="tel"
              ...
              <div>
                <label className="block text-stone-600 mb-1">{t("GSTIN Number")}</label>
                <input
                  type="text"
                  placeholder={t("15-digit GSTIN")}
                  value={newSuppGstin}
